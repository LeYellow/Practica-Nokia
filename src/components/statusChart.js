...
const StatusChart = () => {
  ...
  const fetchStatus = async ()=> {
    await axios.get("http://localhost/Tickets2/tickets/src/backend/getStatusCounter.php").then(
      (response) => {
  ...
  const fetchTicketsByStatus = async (status)=> {
    await axios.get(`http://localhost/Tickets2/tickets/src/backend/getTicketsByStatus.php?status=${status}`).then(
      (response) => {
        console.log(response);
        if(Array.isArray(response.data)){
          if(response.data.length>0 && typeof response.data[0] === 'object'){
